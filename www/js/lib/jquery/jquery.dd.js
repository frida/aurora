   ...
   var dd = function(element, options)
   {
	...
	var createA = function(currentOptOption, current, currentopt, tp) {
		...
		if(options.useSprite!=false) {
		 ...
		 var opt = $(currentOptOption);
		 var pixels = opt.attr("data-pixels");
		 if (pixels) {
			 pixels = atob(pixels);
			 var w = opt.attr("data-width");
			 var h = opt.attr("data-height");
		...
		if (!usingCanvas) {
			...
			if(options.jsonTitle==true && isJson==true) {			 
				 if(arrow.length!=0) {
					 ...
					 var t = (typeof obj[0].title=="undefined") ? "" : obj[0].title;
	...
	var setTitleText = function (sText) {
		var titletextid = getPostID("postTitleTextID");
		var el = $("#"+titletextid).html(sText);		
		var canvas = el.find("canvas");
		if (canvas.length > 0) {
			var originalCanvas = $("#" + canvas.attr("data-option")).find("canvas").get(0);
			var imageData = originalCanvas.getContext('2d').getImageData(0, 0, originalCanvas.width, originalCanvas.height);
			canvas.get(0).getContext('2d').putImageData(imageData, 0, 0);
		}
	};
